...
/*Updating Database. https://github.com/TylerC10/CS290-Web-Development/tree/master/Database%20Interactions%20and%20UI*/
/*MySQL Date and Time Functions. Oracle. 
...
app.get('/update',function(req,res,next){
  var context = {};
  pool.query("SELECT * FROM workoutLog WHERE id=?", [req.query.id], function(err, result){
    if(err){
      next(err);
      return;
    }
    if(result.length == 1){
      ...
      pool.query("UPDATE workoutLog SET name=?, reps=?, weight=?, date=?, pounds=? WHERE id=? ",
